...
4. In project settings → Custom domains → add `attestly.org` and `www.attestly.org`. Cloudflare auto-configures the DNS since the domain is registered with them.
...
3. Add `attestly.org` as the custom domain
4. Configure DNS at Cloudflare: CNAME `attestly.org` → `attestly.github.io`
...
- The footer "Specifications" links currently point to `#` — wire them to `https://spec.attestly.org (TBD)/` once that subdomain is up.
